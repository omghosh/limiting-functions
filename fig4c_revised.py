...
plt.rcParams['xtick.labelsize'] = 13.6

plt.rcParams['ytick.major.size'] = 5
plt.rcParams['ytick.minor.size'] = 2
plt.rcParams['ytick.major.width'] = 1
plt.rcParams['ytick.minor.width'] = 1
plt.rcParams['ytick.labelsize'] = 13.6
...
for metric in ['ind_detection_limit', 'ind_entropy', 'overall_detection_limit', 'overall_entropy']:
        ...
        if genotype in dimensionality_results_evo2D.index:
            ...
            for env in dimensionality_results_evo2D.keys():
                if env == '2Day':
                    ...
                    plt.scatter(x, y1, color = gene_colors[genotype], label = genotype,  s = 20,  alpha=0.75, marker = 's')
                    plt.scatter(x, y2, color = gene_colors[genotype],  label = genotype, s = 20, alpha=0.75, marker='s')
        ...
        if genotype in (dimensionality_results_evo1D.index):
            ...
            for env in environments: 
                ...
                if env == '1Day':
                    ...
                    plt.scatter(x, y1, color = gene_colors[genotype], label = genotype, s = 20, alpha = 0.75, marker = 'D')
                    plt.scatter(x, y2, color = gene_colors[genotype],  label = genotype, s =20, alpha = 0.75, marker = 'D')
    ...
    custom_handles = [
        Line2D([], [], color='gray', marker='s', linestyle='None', 
            markersize=4, markerfacecolor='None', label='Evo2D'),
        Line2D([], [], color='gray', marker='D', linestyle='None',
            markersize=4, markerfacecolor='None', label='Evo1D')
    ]
    ...
    plt.xlabel('Evolution Base Dimensionality')
    plt.ylabel('Non-Evolution Base Dimensionality')
    plt.xlim(min_dim-0.5, max_dim+0.5)
    plt.ylim(min_dim-0.5, max_dim+0.5)
    plt.gcf().subplots_adjust(left=0.15, right=0.95, bottom=0.15, top=0.95)

    # plt.title(f'{metric} Dimensionality')
    # plt.tight_layout()
    if metric == 'ind_detection_limit':
        plt.savefig(f'plots/fig4c_revised.pdf')
    else:
        plt.savefig(f'plots/SI/{metric}_dimensionality_scatterplot.pdf')
    # plt.show()
