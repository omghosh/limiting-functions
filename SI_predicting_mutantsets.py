...
mutant_names = ['original', 'anc: IRA1_NON', 'anc: IRA1_MIS', 'anc: GPB2', 'anc: TOR1', 'anc: CYR1']
...
for mut in mutant_names:
    ...
    for focal_base, target_base1, target_base2 in [('Salt', '1Day', '2Day'), ('1Day', '2Day', 'Salt'), ('2Day', 'Salt', '1Day')]:
        ...
        plt.tight_layout()
        if focal_base == '2Day':
            plt.savefig(f'plots/SI/fig5_b_{mut}.png', dpi=300)
        elif focal_base == 'Salt':
            plt.savefig(f'plots/SI/fig5_c_salt_{mut}.png', dpi=300)
        elif focal_base == '1Day':
            plt.savefig(f'plots/SI/fig5_c_1day_{mut}.png', dpi= 300)
        # plt.show()
        plt.close()
