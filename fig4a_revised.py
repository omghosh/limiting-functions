...
for base_env in environment_dict.keys():
    ...
    plt.semilogy((S[:16]**2)/np.sum(S**2), 'o-', label = base_env, color=env_color_dict[base_env], markerfacecolor = 'none', markeredgecolor=env_color_dict[base_env])
...
plt.title('Ancestor: WT, Evolution Condition: 2 Day')
plt.xlabel('Component')
plt.ylabel('Fraction of Variance Explained')
# plt.legend(fontsize = 10)
plt.xticks(ticks=np.arange(0, 16, 2), labels=np.arange(1, 17, 2)) 
plt.savefig(f'plots/fig4a_revised.pdf')
# plt.show()
