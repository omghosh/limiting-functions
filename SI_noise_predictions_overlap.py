...
# std_cols = [c.replace('fitness', 'stderror') for c in all_conds]
# std_values = organized_perturbation_fitness_df.loc[:, std_cols].values
# noise_array = np.random.normal(size=std_values.shape) * std_values
# noise_df = pd.DataFrame(noise_array, columns=all_conds,
#                         index=organized_perturbation_fitness_df.index)
# print(noise_df)
...
for env in all_conds:
    ...
    if base == '2Day':
        ...
        if r1_label in fitness_df.columns and r2_label in fitness_df.columns:
            ...
            noise_df[env]=(r1_delta_fitness-r2_delta_fitness)
    elif base == '1Day':
...
for mut in mutant_names[:1]:
    ...
    for focal_base, target_base1, target_base2 in [('Salt', '1Day', '2Day'), ('1Day', '2Day', 'Salt'), ('2Day', 'Salt', '1Day')]:
        ...
        X1 = noise_df.loc[which_mutants, environment_dict[focal_base]].values
        X2 = noise_df.loc[which_mutants, environment_dict[target_base1]].values
        X3 = noise_df.loc[which_mutants, environment_dict[target_base2]].values
        ...
        plt.tight_layout()
        if focal_base == '2Day':
            plt.savefig(f'plots/SI/noise_preds_2Day.png', dpi=600)
        elif focal_base == 'Salt':
            plt.savefig(f'plots/SI/noise_preds_Salt.png', dpi=600)
        elif focal_base == '1Day':
            plt.savefig(f'plots/SI/noise_preds_1Day.png', dpi= 600)
        # plt.show()
        plt.close()
